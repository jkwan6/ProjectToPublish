precision mediump float;
varying float vRandom;

uniform vec3 uColor;
uniform sampler2D uTexture;

varying vec2 vUv;
varying float vElevation;


float random(vec2 st)
{
return fract(sin(dot(st.xy, vec2(12.9898, 78.233))) * 43758.5453123);
}



vec4 permute(vec4 x)
{
return mod(((x * 34.0) + 1.0) * x, 289.0);
}


vec2 fade(vec2 t) {return t*t*t*(t*(t*6.0-15.0)+10.0);}

float cnoise(vec2 P){
  vec4 Pi = floor(P.xyxy) + vec4(0.0, 0.0, 1.0, 1.0);
  vec4 Pf = fract(P.xyxy) - vec4(0.0, 0.0, 1.0, 1.0);
  Pi = mod(Pi, 289.0); // To avoid truncation effects in permutation
  vec4 ix = Pi.xzxz;
  vec4 iy = Pi.yyww;
  vec4 fx = Pf.xzxz;
  vec4 fy = Pf.yyww;
  vec4 i = permute(permute(ix) + iy);
  vec4 gx = 2.0 * fract(i * 0.0243902439) - 1.0; // 1/41 = 0.024...
  vec4 gy = abs(gx) - 0.5;
  vec4 tx = floor(gx + 0.5);
  gx = gx - tx;
  vec2 g00 = vec2(gx.x,gy.x);
  vec2 g10 = vec2(gx.y,gy.y);
  vec2 g01 = vec2(gx.z,gy.z);
  vec2 g11 = vec2(gx.w,gy.w);
  vec4 norm = 1.79284291400159 - 0.85373472095314 * 
    vec4(dot(g00, g00), dot(g01, g01), dot(g10, g10), dot(g11, g11));
  g00 *= norm.x;
  g01 *= norm.y;
  g10 *= norm.z;
  g11 *= norm.w;
  float n00 = dot(g00, vec2(fx.x, fy.x));
  float n10 = dot(g10, vec2(fx.y, fy.y));
  float n01 = dot(g01, vec2(fx.z, fy.z));
  float n11 = dot(g11, vec2(fx.w, fy.w));
  vec2 fade_xy = fade(Pf.xy);
  vec2 n_x = mix(vec2(n00, n01), vec2(n10, n11), fade_xy.x);
  float n_xy = mix(n_x.x, n_x.y, fade_xy.y);
  return 2.3 * n_xy;
}



void main()
{
    vec4 textureColor = texture2D(uTexture, vUv);
    textureColor.rgb *= vElevation * 2.0 + 0.9;
    gl_FragColor = textureColor;


//    // Pattern 3
//    float strenght = vUv.x;
//    gl_FragColor = vec4(vUv.x,vUv.x, vUv.x, 1.0);


//    // Pattern 4
//    float strength = vUv.y;
//    gl_FragColor = vec4(strength,strength, strength, 1.0);

//    // Pattern 5
//    float strength = 1.0 - vUv.y;
//    gl_FragColor = vec4(strength,strength, strength, 1.0);

//    // Pattern 6
//    float strength = vUv.y * 10.0;
//    gl_FragColor = vec4(strength,strength, strength, 1.0);

//    // Pattern 6
//    float strength = step(0.8, mod(vUv.y, 0.1) * 10.0);
//    gl_FragColor = vec4(strength,strength, strength, 1.0);

//    // Pattern 7
//    float strength = step(0.8, mod(vUv.x, 0.1) * 10.0);
//    gl_FragColor = vec4(strength,strength, strength, 1.0);

//    // Pattern 11
//    float strengthx = step(0.8, mod(vUv.x, 0.1) * 10.0);
//    float strengthy = step(0.8, mod(vUv.y, 0.1) * 10.0);
//    float strength = strengthx + strengthy;
//    gl_FragColor = vec4(strength,strength, strength, 1.0);

//    // Pattern 12
//    float strengthx = step(0.8, mod(vUv.x, 0.1) * 10.0);
//    float strengthy = step(0.8, mod(vUv.y, 0.1) * 10.0);
//    float strength = strengthx * strengthy;
//    gl_FragColor = vec4(strength,strength, strength, 1.0);

//    // Pattern 13
//    float strengthy1 = step(0.8, mod(vUv.y * 10.0 + 0.2, 1.0));
//    float strengthx1 = step(0.4, mod(vUv.x * 10.0, 1.0));
//    float strengthy2 = step(0.4, mod(vUv.y * 10.0, 1.0));
//    float strengthx2 = step(0.8, mod(vUv.x * 10.0 + 0.2, 1.0));
//    float strength1 = strengthy1 * strengthx1;
//    float strength2 = strengthy2 * strengthx2;
//    float strength = strength1 + strength2;

//    // Pattern 16
//    float strengthx = abs(0.5 - vUv.x);
//    float strengthy = abs(0.5 - vUv.y);
//    float strength = min( strengthx , strengthy);


//    // Pattern 16
//    float strengthx = abs(0.5 - vUv.x);
//    float strengthy = abs(0.5 - vUv.y);
//    float strength = max( strengthx , strengthy);

//    // Pattern 16
//    float strengthx = abs(0.5 - vUv.x);
//    float strengthy = abs(0.5 - vUv.y);
//    float strength = step(0.2,max( strengthx , strengthy));


//    // Pattern 16
//    float strengthx = abs(0.5 - vUv.x);
//    float strengthy = abs(0.5 - vUv.y);
//    float strength1 = step(0.2,max( strengthx , strengthy));
//    float strength2 = 1.0 - step(0.25,max( strengthx , strengthy));
//    float strength = strength1 * strength2;

//    // Pattern 16
//    float strength = round(vUv.x);


//    // Pattern 16
//    float strength = floor((vUv.x * 10.0)) / 10.0;


//    // Pattern 16
//    float strengthx = floor((vUv.x * 10.0)) / 10.0;
//    float strengthy = floor((vUv.y * 10.0)) / 10.0;
//    float strength = strengthx * strengthy;

//    // Pattern 17
//    vec2 gridUv = vec2(
//    floor((vUv.x * 10.0)) / 10.0,
//    floor((vUv.y * 10.0)) / 10.0
//    );
//    float strength = random(gridUv);


//    // Pattern 17
//    vec2 gridUv = vec2(
//    floor((vUv.x * 10.0)) / 10.0,
//    floor(((vUv.y + vUv.x) * 10.0)) / 10.0
//    );
//    
//    float strength = random(gridUv);

//    // Pattern 17
//    float strength = length(vUv);

//    // Pattern 18
//    float strength = 1.0-distance(vUv, vec2(0.5));

//    // Pattern 18 - SUN
//    float strength = 0.01 / distance(vUv, vec2(0.5));


//    // Pattern 19- SUN
//    vec4 textureColor = texture2D(uTexture, vUv);
//    textureColor.rgb *= vElevation * 2.0 + 0.9;
//    gl_FragColor = textureColor;
//
//
//    float strength = sin(cnoise(vUv * 10.0) * 20.0);


  float strength = cnoise(vUv);

//    gl_FragColor = vec4(strength,strength, strength, 1.0);
}
